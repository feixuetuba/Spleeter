...
class Separator(object):
    def __init__(self, pretrain_model, instruments, frame_length, frame_step,
                 segment_length, frequency_bins, separation_exponent, mask_extension,device="cuda"):
        ...
        sd = torch.load(self.pretrain_model, map_location='cpu')
        model = SpleeterNet(self.instruments, False)
    ...
    def separate(self, wave):
        ...
        with torch.no_grad():
            masks = self.model(spectrogram.to(self.device))
